...
sub new {
  ...
  # Labels let you give names to locations of particular instructions.
  $self->{_labels}  = undef;
    ...
    if ( grep { $_ eq $mnemonic } @instructions ) {
      ...
      if ( grep { $_ eq $mnemonic } @with_address ) {
        ...
        if ($operand =~ /[A-Z]\d{3}/) {
          ...
          $statement->{$mnemonic}->{label} = $operand;
        };
      }
...
#        eval { $operand = $self->token_kw() } or
        $operand = $self->generic_token( expression => qr/\'.*?\'/ );
        $statement = {
          $mnemonic => {
            type        => 'instruction',
            expression  => $operand,
